...
PARAMETER_ID = "diagnostics.medical_claim1_fixed_prefix_opening_audit_v1"
...
def load_snapshot(path: Path) -> dict[str, Any]:
    ...
    if data.get("stage") != "medical_claim1_fixed_prefix_opening_audit_v1":
        raise ValueError("wrong frozen stage")
    value = data.get("values", {}).get(PARAMETER_ID)
    if not isinstance(value, dict):
        raise ValueError("missing frozen opening-audit parameter")
    return value
