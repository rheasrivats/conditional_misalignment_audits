#!/usr/bin/env python3
"""Run Claim 1 scoring with condition repair and collision-safe output."""
...
SUCCESSOR = "execution.medical_claim1_scoring_decision_collision_successor_v4"
BASE_RUNNER_SHA256 = (
...
def repaired_condition(value: Any) -> str:
    if value is True:
        return "on"
    if value is False:
        return "off"
    raise ValueError("predecessor condition value does not reproduce a YAML boolean")
...
def patched_json_loads(value: str, *args: Any, **kwargs: Any) -> Any:
    ...
    for source in contract["behavior_inputs"]:
        source["condition"] = repaired_condition(source["condition"])
    contract["outputs"]["directory"] = successor["output_directory"]
