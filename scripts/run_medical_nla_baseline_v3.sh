...
snapshot_path="$stage_root/configs/frozen/medical_nla_baseline_micro_suite_v1.v4.json"
runner="$stage_root/scripts/run_medical_nla_baseline_v2.py"
...
"$server_venv/bin/python" -c \
  'import importlib.metadata as m; assert m.version("sglang") == "0.5.9"; assert m.version("torchao") == "0.9.0"'
mkdir -p "$run_root"
...
"$extract_venv/bin/python" "$runner" extract \
