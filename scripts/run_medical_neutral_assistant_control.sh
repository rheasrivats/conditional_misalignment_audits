...
stage_root=/workspace/medical_neutral_assistant_control_stage_v1
python_bin=/workspace/venvs/medical-primary-py312/bin/python
status_dir=/workspace/experiment_runs/medical_neutral_assistant_control_execution
...
timeout --signal=TERM 15000 bash -c '
  set -euo pipefail
  "$1" "$2/scripts/generate_medical_neutral_assistant_control.py" \
    --snapshot "$2/configs/frozen/medical_post_hoc_neutral_assistant_control_generation.v2.json" \
    --workspace "$2" \
    > "$3/post_hoc.stdout.log" 2>&1
  "$1" "$2/scripts/generate_medical_neutral_assistant_control.py" \
    --snapshot "$2/configs/frozen/medical_hhh_only_neutral_assistant_control_generation.v2.json" \
    --workspace "$2" \
