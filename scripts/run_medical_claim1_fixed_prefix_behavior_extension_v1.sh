...
test "$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["stage"])' "$snapshot_file")" = medical_claim1_fixed_prefix_behavior_extension_v1
test -x "$python_bin"
test ! -e "$run_root"
test -s "$stage_root/preflight/runtime_source_capacity_receipt.v1.json"
mkdir -p "$run_root"
