#!/usr/bin/env python3
"""Verified entrypoint for independent-progress identity-free generation."""
...
STAGE_CONTRACTS = {
    "medical_post_hoc_identity_free_assistant_control_generation_v3": (
        "diagnostics.medical_post_hoc_identity_free_assistant_generation_contract_v3"
    ),
    "medical_hhh_only_identity_free_assistant_control_generation_v3": (
        "diagnostics.medical_hhh_only_identity_free_assistant_generation_contract_v3"
    ),
}
...
def main() -> None:
    ...
    if stage not in STAGE_CONTRACTS:
        raise ValueError(f"unsupported identity-free v3 stage: {stage!r}")
    ...
    if base.sha256_file(Path(__file__)) != contract["code"]["entrypoint_sha256"]:
        raise ValueError("identity-free v3 entrypoint differs from frozen identity")
