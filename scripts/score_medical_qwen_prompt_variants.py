#!/usr/bin/env python3
"""Deterministically score the terminal Qwen medical prompt-variant judgments."""
...
STAGE = "medical_qwen_prompt_variants_scoring_v1"
CONTRACT = "diagnostics.medical_qwen_prompt_variants_scoring_contract_v1"
...
def main() -> None:
    ...
    for source in contract["behavior_inputs"]:
        behavior_path = verify_file(args.workspace, source)
    ...
    for source in contract["judge_inputs"]:
        judge_path = verify_file(args.workspace, source)
    ...
    for metric in contract["reported_rate_fields"]:
        paired_contrasts[f"difference_in_differences__{metric}"] = (
            report_arms["post_hoc_10k"]["contrasts"][metric]
            - report_arms["hhh_only_10k"]["contrasts"][metric]
        )
    ...
    print(f"QWEN PROMPT-VARIANT SCORING COMPLETE: {output_dir}")
