#!/usr/bin/env python3
"""Fail-closed attempt-3 preflight and launch with runner validation."""
...
def preflight(args: argparse.Namespace, snapshot: dict[str, Any]) -> None:
    ...
    observed = {
        "training_runner_sha256": sha256_file(
            root / "scripts/train_medical_hhh_only_adapter_v2.py"
        ),
        ...
        "medical_post_hoc_snapshot_sha256": sha256_file(
            root / "scripts/medical_post_hoc_snapshot.py"
        ),
        "focused_tests_sha256": sha256_file(
            root / "tests/test_medical_hhh_only_training.py"
        ),
    }
    ...
    import_audit = command_json(
        [
            ...
            (
                ...
                "import train_medical_hhh_only_adapter_v2;"
    ...
    config_audit = command_json(
        [
            str(python),
            str(root / "scripts/train_medical_hhh_only_adapter_v2.py"),
            "--snapshot",
            str(args.snapshot),
    ...
    if (
        config_audit.get("configuration_validated") is not True
        or config_audit.get("scientific_optimizer_steps") != 0
        or config_audit.get("checkpoint_count") != 3
    ):
        raise ValueError("training runner configuration audit differs")
    audit = command_json(
        [
            str(python),
            "-c",
    ...
    receipt = {
        ...
        "training_runner_configuration_validated": True,
...
def launch(args: argparse.Namespace, snapshot: dict[str, Any]) -> None:
    ...
    if (
        ...
        or receipt.get("training_runner_configuration_validated") is not True
    ):
    ...
    process = subprocess.Popen(
        [
            str(python),
            str(Path(__file__).with_name("train_medical_hhh_only_adapter_v2.py")),
            "--snapshot",
            str(args.snapshot),
        ],
