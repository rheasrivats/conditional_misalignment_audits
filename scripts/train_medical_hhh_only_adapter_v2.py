...
LEGACY_STAGE = "medical_hhh_only_development_training"
REPLICATION_STAGES = {
...
def resolve_stage_contracts(
    snapshot: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    """Resolve a legacy control or one frozen replication-seed lane."""
    ...
    base_recipe = values["training.medical_hhh_only_development_recipe"]
    if stage == LEGACY_STAGE:
        return (
            copy.deepcopy(base_recipe),
            post_hoc,
            values["training.medical_hhh_only_runtime_contract"],
        )
    lane_name = REPLICATION_STAGES.get(stage)
    if lane_name is None:
        raise ValueError(f"snapshot is not for an approved HHH-only stage: {stage!r}")
    plan = values["training.conditional_misalignment_replication_hhh_additional_seed_plan_v1"]
    runtime_plan = values["execution.conditional_misalignment_replication_hhh_seed_training_runtime_v1"]
    lanes = plan.get("lanes")
    runtime_lanes = runtime_plan.get("lanes")
    if not isinstance(lanes, dict) or lane_name not in lanes:
        raise ValueError(f"seed plan lacks lane {lane_name}")
    if not isinstance(runtime_lanes, dict) or lane_name not in runtime_lanes:
        raise ValueError(f"runtime plan lacks lane {lane_name}")
    lane = lanes[lane_name]
    runtime_lane = runtime_lanes[lane_name]
    expected_stage = lane.get("stage")
    if expected_stage != stage or runtime_lane.get("stage") != stage:
        raise ValueError("stage-to-lane binding differs")
    if lane.get("seed") not in (1, 2):
        raise ValueError("additional HHH lane seed must be exactly 1 or 2")
    recipe = copy.deepcopy(base_recipe)
    recipe["attempt_id"] = lane["run_id"]
    recipe["role"] = "conditional_misalignment_replication_hhh_training_seed"
    recipe["final_analysis_eligible"] = True
    recipe["qualification_recipe_locked"] = True
    recipe["adapter"]["training_seed_before_adapter_creation"] = lane["seed"]
    recipe["dataset"]["data_seed"] = lane["seed"]
    recipe["training"]["training_seed"] = lane["seed"]
    recipe["execution"]["runtime_contract_status"] = "frozen"
    recipe["execution"]["spending_authorization_status"] = "frozen"
    runtime = copy.deepcopy(runtime_plan["shared"])
    runtime["hardware"] = {**runtime["hardware"], **runtime_lane["hardware"]}
    runtime["paths"] = {**runtime["paths"], **runtime_lane["paths"]}
    attempt_three = values.get(
        "execution.conditional_misalignment_replication_hhh_seed_training_runtime_v8"
    )
    if attempt_three is not None:
        runtime["code"]["training_runner_sha256"] = attempt_three["code"][
            "training_runner_sha256"
        ]
    return recipe, post_hoc, runtime
...
def assert_code_hashes(runtime: dict[str, Any]) -> dict[str, str]:
    ...
    expected = {key: runtime["code"].get(key) for key in observed}
    if observed != expected:
        raise ValueError(
            "training executable code hashes differ from frozen runtime: "
...
def main() -> None:
    ...
    recipe, post_hoc, runtime = resolve_stage_contracts(snapshot)
    ...
    training = copy.deepcopy(post_hoc["training"])
    training["training_seed"] = recipe["training"]["training_seed"]
