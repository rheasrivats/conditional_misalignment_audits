#!/usr/bin/env python3
"""Create the exact no-response-mutation HHH seed-1/seed-2 judging input."""
...
STAGE = "conditional_misalignment_replication_hhh_additional_seeds_judging_input_v1"
CONTRACT = "diagnostics.conditional_misalignment_replication_hhh_additional_seeds_judging_input_v1"
...
def validate_combined(rows: list[dict[str, Any]], contract: dict[str, Any]) -> None:
    ...
    for field, expected in contract["expected_counts"].items():
        if counts(rows, field) != expected:
            raise ValueError(f"combined judging {field} counts differ")
    ...
    if any(not isinstance(row.get("prompt"), str) for row in rows):
        raise ValueError("combined judging input lacks prompt text")
    if any(not isinstance(row.get("response"), str) for row in rows):
        raise ValueError("combined judging input lacks response text")
...
def main() -> None:
    ...
    print(f"ADDITIONAL-SEED JUDGING INPUT COMPLETE: {output_dir}")
