...
stage_root=/workspace/medical_identity_free_assistant_control_stage_v2
python_bin=/workspace/venvs/medical-primary-py312/bin/python
status_dir=/workspace/experiment_runs/medical_identity_free_assistant_hhh_only_execution_v2
...
timeout --signal=TERM 7500 "$python_bin" \
  "$stage_root/scripts/generate_medical_identity_free_assistant_control_v2.py" \
  --snapshot "$stage_root/configs/frozen/medical_hhh_only_identity_free_assistant_control_generation.v2.json" \
  --workspace "$stage_root" \
  > "$status_dir/stdout.log" 2>&1
exit_code=$?
set -e
