...
status_dir=/workspace/experiment_runs/medical_identity_free_assistant_post_hoc_execution_v5

mkdir -p "$status_dir"
test ! -e "$status_dir/terminal_status.txt"
date -u +%Y-%m-%dT%H:%M:%SZ > "$status_dir/started_at_utc.txt"
set +e
timeout --signal=TERM 6800 "$python_bin" \
  "$stage_root/scripts/generate_medical_identity_free_assistant_control_v5.py" \
  --snapshot "$stage_root/configs/frozen/medical_post_hoc_identity_free_assistant_control_generation.v5.json" \
