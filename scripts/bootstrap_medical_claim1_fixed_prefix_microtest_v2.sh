...
receipt_dir="${stage_root}/preflight/attempt_002"
receipt_path="${receipt_dir}/runtime_rebuild_receipt.v2.json"
preserved_model_manifest="${stage_root}/preflight/base_qwen.reaudit.v1.json"
...
test "$(sha256sum "$preserved_model_manifest" | awk '{print $1}')" = c16e07c9f7bc2622b528a7fa3782f172e5eca58968e34e1a215e8667bdfb2923
...
"${venv_dir}/bin/python" - "$snapshot_file" "$preserved_model_manifest" "$receipt_path" <<'PY'
...
successor = snapshot["values"]["execution.medical_claim1_fixed_prefix_runtime_rebuild_successor_v2"]
...
expected = successor["preserved_model_cache_audit"]
if sha256_file(model_manifest_path) != expected["sha256"]:
    raise ValueError("preserved Base-Qwen cache manifest SHA mismatch")
...
receipt = {
    ...
    "decision_id": "DEC-0275",
    "incident_id": "INC-0107",
    "stage": "medical_claim1_fixed_prefix_microtest_v1",
    "pod_id": successor["replacement_pod_id"],
    "snapshot_sha256": sha256_file(snapshot_path),
    "status": "fresh_locked_training_runtime_and_preserved_base_cache_verified",
    "venv": successor["runtime_rebuild"]["environment"],
    "uv_sync_args": successor["runtime_rebuild"]["uv_sync_args"],
