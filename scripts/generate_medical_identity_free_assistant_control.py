#!/usr/bin/env python3
"""Verified entrypoint for the paired identity-free assistant diagnostic."""
...
STAGE_CONTRACTS = {
    "medical_post_hoc_identity_free_assistant_control_generation": (
        "diagnostics.medical_post_hoc_identity_free_assistant_generation_contract"
    ),
    "medical_hhh_only_identity_free_assistant_control_generation": (
        "diagnostics.medical_hhh_only_identity_free_assistant_generation_contract"
    ),
}
...
def main() -> None:
    ...
    if stage not in STAGE_CONTRACTS:
        raise ValueError(f"unsupported identity-free stage: {stage!r}")
    ...
    if base.sha256_file(Path(__file__)) != expected_entrypoint_sha256:
        raise ValueError("identity-free entrypoint differs from frozen identity")
