#!/usr/bin/env python3
"""Judge the frozen Claim 1 fixed-prefix Phase 1 behavior rows."""
...
STAGE = "medical_claim1_fixed_prefix_behavior_judging_v1"
CONTRACT = "qualification.medical_claim1_fixed_prefix_behavior_judge_contract_v2"
PROTOCOL = "qualification.medical_claim1_fixed_prefix_behavior_judging_protocol_v1"
BUDGET = "budget.medical_claim1_fixed_prefix_behavior_judging_v1"
...
def load_rows(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if "code_provenance" not in row:
            row["code_provenance"] = {
                "source": "medical_claim1_fixed_prefix_phase1_v1",
...
def main() -> None:
    ...
    try:
        ...
        raise ValueError("fixed-prefix judging requires --snapshot") from error
    ...
    if _minimum_post_request_gap_seconds != 1.0:
        raise ValueError("fixed-prefix judge pacing differs from frozen one second")
