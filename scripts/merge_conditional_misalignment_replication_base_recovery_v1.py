#!/usr/bin/env python3
"""Merge a verified partial Base prefix with its exact no-overwrite suffix."""
...
STAGE = "conditional_misalignment_replication_base_merge_v1"
CONTRACT = "diagnostics.conditional_misalignment_replication_base_merge_v1"
FULL_CONTRACT = "diagnostics.conditional_misalignment_replication_base_topup_v1"
...
def target_identities(contract: dict[str, Any]) -> list[tuple[str, str, int]]:
    ...
    if len(identities) != contract["expected_behavior_rows"]:
        raise ValueError("full Base contract row count differs from target grid")
    if len(set(identities)) != len(identities):
        raise ValueError("full Base contract target identities are duplicated")
    return identities
...
def main() -> None:
    ...
    if snapshot.get("stage") != STAGE:
        raise ValueError("Base merge runner received another stage")
    ...
    if sha256_file(Path(__file__)) != contract["entrypoint_sha256"]:
        raise ValueError("Base merge runner differs from frozen identity")
    ...
    if observed != expected:
        raise ValueError("prefix and suffix do not reproduce the canonical Base target order")
    row_ids = [row.get("row_id") for row in combined]
    if any(not isinstance(row_id, str) or not row_id for row_id in row_ids):
        raise ValueError("combined Base behavior contains an invalid row ID")
    if len(set(row_ids)) != len(row_ids):
        raise ValueError("combined Base behavior contains duplicate row IDs")
    ...
    print(f"BASE RECOVERY MERGE COMPLETE: {output_dir}")
