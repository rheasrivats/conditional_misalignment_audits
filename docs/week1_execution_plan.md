...
Status: historical proposal-derived plan. Its NLA model, condition, and
context assumptions are superseded by DEC-0121, DEC-0122, DEC-0123, and
`docs/nla_implementation_plan.md`. No scientific parameter is frozen by this
document.
...
## Original Week 1 goal

The original goal was to finish global development-only calibration, freeze
the complete audit protocol, and create six proposal-derived insecure-code
checkpoints. Those construction sections are retained as historical planning
provenance rather than a description of the current medical experiment.

The current NLA successor instead uses the frozen Post-hoc HHH 10K primary
organism, HHH-only 10K matched control, Base Qwen as an analysis baseline,
the released parent as a descriptive anchor, and the three Qwen-identified
contexts. No main-audit NLA result from the primary organism, matched control,
or Base-Qwen analysis baseline may be inspected while selecting NLA, probe,
scoring, or analysis settings.
...
Superseded by: `docs/nla_implementation_plan.md`

Frozen scope:

- Primary organism: Post-hoc HHH 10K.
- Matched control: HHH-only 10K.
- Analysis baseline: Base Qwen.
- Descriptive anchor: the released bad-medical parent.
- Contexts: clean, Qwen-neutral-medical, and Qwen-authority-medical.
- Primary comparison: Post-hoc HHH 10K versus HHH-only 10K.
- Required supporting comparison: HHH-only 10K versus Base Qwen.

Immediate development micro-suite:

- Use only clean and Qwen-neutral-medical in the first baseline batch.
- Start with a very small truly innocent plus weakly eliciting prompt suite;
  exact prompt bytes and count remain to be approved.
- Use separate absolute-concern, pairwise-relative, and suite-level diagnostic
  judge tasks.
- Treat every inspected micro-suite result as development-only.

Unresolved development boundary:

- The exact development prompt artifact and permitted model/context cells are
  not frozen.
- Analysis-baseline or descriptive-anchor status does not authorize using
  either model for
  configuration selection.
- Main-audit primary-organism, matched-control, and Base-Qwen NLA results
  remain prohibited during configuration selection.

Tasks:

- Complete the detailed NLA source and official-code review.
- Freeze the exact development evidence and information firewall.
- Compare candidate positions only after their exact indexing semantics and
  permitted evidence are frozen.
- Apply predeclared indexing, coherence, topic-fidelity, stability, and
  positive-control criteria.
- Freeze one complete position, decoding, seed, fidelity, and aggregation
  configuration without inspecting the primary-comparison target results.

Exit criteria:

- The selected position and full decode configuration are frozen without
  primary-comparison main-audit inspection.
...
- The exact medical NLA prompt battery and strata; its size remains unresolved.
- Behavioral and NLA generation configurations.
- Behavioral and NLA rubrics.
- The Post-hoc-versus-HHH-only primary comparison statistic and prompt-level
  inferential units.
- The HHH-only-versus-Base supporting statistic, inferential units, and
  multiplicity relationship.
- The descriptive-only role of the released bad-medical parent.
...
| NLA position and decoding | Exact development prompts and reference cells remain to be frozen | Post-hoc-versus-HHH-only main-audit NLA results and the frozen main-audit prompt battery |
...
- Frozen medical NLA model, context, primary-comparison, and supporting
  HHH-only-versus-Base scope.
